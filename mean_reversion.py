...
import warnings
import matplotlib
matplotlib.use('Agg')  # Esto evita errores al generar gráficos en servidores sin pantalla
...
if data.empty:
    ...
    # Convertir el índice a la zona horaria de Colombia
    data.index = data.index.tz_convert(colombia_tz)

    # 3. Cálculo de métricas y anomalías
    ...
    # Añadir la columna de señal de trading
...
ax1.set_title(f'Análisis de Anomalías Horarias – {ticker} (Hora Colombia)', fontsize=14)
...
# Añadir la columna de señal de trading a df_features
...
# Feature 2: Volatilidad Histórica (Rolling Std de retornos)
df_features['Volatility'] = df_features['log_return'].rolling(window=24).std()

# Feature 3: Distancia respecto a la Media Móvil (Momentum)
...
# Como yf a veces da problemas con volumen, usaremos la aceleración del log_return
df_features['Log_Return_Accel'] = df_features['log_return'].diff()

# Añade features rezagadas
...
# Limpiamos los NaNs generados por las ventanas móviles
...
# 2.2. Creación de columnas Target
...
# Eliminamos las últimas filas que quedan con NaN en los targets
...
# También probamos Random Forest como alternativa
param_grid_rf = {
...
for h in horizontes:
    ...
    print(f"📊 Train: {len(X_train)} observaciones | Test: {len(X_test)} observaciones")
    ...
    print("\n🌲 Probando Random Forest con Grid Search...")
    ...
    print(f"\n🏆 GANADOR: {modelo_nombre}")
    print(f"📋 Parámetros: {mejores_params}")

    # ============================================
    # Métricas detalladas en test set
    # ============================================
    ...
    print(f"{'Métrica':<20} {'Valor':<12} {'vs Baseline':<12}")
    print(f"{'-'*60}")
    ...
    print(f"{'Baseline (mayoría)':<20} {baseline:<12.4f}")

    # ============================================
    # Re-entrenar con TODOS los datos para producción
    # ============================================
    print(f"\n♻️  Re-entrenando {modelo_nombre} con dataset completo...")
...
for h in horizontes:
    mejora = metricas_detalladas[h]['mejora_vs_baseline']
    if mejora < 0.02:
        print(f"⚠️  Horizonte {h}h: Mejora mínima ({mejora*100:.2f}%). Considerar no operar este horizonte.")

    if metricas_detalladas[h]['test_accuracy'] < 0.52:
        print(f"⚠️  Horizonte {h}h: Accuracy muy bajo ({metricas_detalladas[h]['test_accuracy']:.2%}). No recomendado para trading real.")

print("\n💡 RECOMENDACIÓN:")
print("Solo opera en horizontes con mejora_vs_baseline > 2% y accuracy > 52%")
print("Usa KNN_score >= 60% como filtro adicional para señales")

import numpy as np
import pandas as pd

# ============================================
# 4. PREDICCIÓN EN TIEMPO REAL (ÚLTIMA VELA)
# ============================================

print("=" * 60)
print("ANÁLISIS DE CONDICIÓN ACTUAL")
print("=" * 60)
...
print(f"📊 Estado: {'🚨 ANOMALÍA DETECTADA' if es_anomalia_actual else '✅ Mercado Normal'}")
...
if es_anomalia_actual:
    ...
    print(f"🎯 Señal Mean Reversion: {signal_mr}")
    print(f"\n{'='*60}")
    ...
    for h in horizontes:
        ...
        if h in metricas_detalladas:
            ...
            confiable = True  # Si no hay métricas, asumimos que sí
        ...
        predicciones[h] = {
            ...
            "vs MeanRev": "✅ Alineado" if compatible else "⚠️ Divergente",
            "Modelo Confiable": "✅" if confiable else "❌"
        }
    ...
    # Consenso general
    print(f"\n{'='*60}")
    print("CONSENSO Y RECOMENDACIÓN")
    print(f"{'='*60}")
    ...
    if consenso_pct >= 80:
        consenso = f"🟢 FUERTE ALCISTA ({consenso_pct:.0f}%)"
    elif consenso_pct >= 60:
        consenso = f"🟢 ALCISTA ({consenso_pct:.0f}%)"
    elif consenso_pct >= 40:
        consenso = f"🟡 NEUTRAL ({consenso_pct:.0f}%)"
    elif consenso_pct >= 20:
        consenso = f"🔴 BAJISTA ({consenso_pct:.0f}%)"
    else:
        consenso = f"🔴 FUERTE BAJISTA ({consenso_pct:.0f}%)"

    print(f"Consenso KNN: {consenso}")
    print(f"Señal Mean Reversion: {signal_mr}")

    # Recomendación de trading
    compatible_count = sum(1 for p in predicciones.values() if p["vs MeanRev"] == "✅ Alineado")
    ...
    if alineamiento >= 60 and consenso_pct not in range(40, 61):
        ...
        print(f"   Dirección sugerida: {signal_mr}")
    else:
        print(f"\n💡 RECOMENDACIÓN: ⚠️ ESPERAR")
        print(f"   Razón: Bajo alineamiento ({alineamiento:.0f}%) o señal neutral")

else:
    print("\n💡 No se requiere análisis detallado.")
    print("   El mercado opera dentro de parámetros normales.")

# ============================================
# 5. ANÁLISIS HISTÓRICO DE ANOMALÍAS
# ============================================

print("\n" + "=" * 60)
print("ANÁLISIS HISTÓRICO - SOLO ANOMALÍAS")
print("=" * 60)

anomalias_historicas = df_features[df_features['anomaly']].copy()
print(f"\n📊 Total de anomalías detectadas: {len(anomalias_historicas)}")
print(f"📊 Porcentaje del dataset: {len(anomalias_historicas)/len(df_features)*100:.2f}%")
...
# ============================================
# 6. REPORTE FINAL CON MÉTRICAS CORREGIDAS
# ============================================
...
# ============================================
# 6.1 ALINEAMIENTO KNN vs MEAN REVERSION POR ANOMALÍA
# ============================================

print("\n" + "=" * 60)
print("ALINEAMIENTO KNN vs MEAN REVERSION POR ANOMALÍA")
print("=" * 60)
...
print("MR_Alignment_Score añadido al reporte final. Este score indica el porcentaje de horizontes en los que el KNN predice la misma dirección que la señal de Mean Reversion para esa anomalía especifica.")

# Categorizar el MR_Alignment_Score
bins = [0, 21, 41, 61, 81, 101]
labels = ['⚪ Basura','🔴Bajo', '🟠Medio', '🟡Bueno', '🟢Excelente']
reporte_final['MR_Alignment_Category'] = pd.cut(reporte_final['MR_Alignment_Score'], bins=bins, labels=labels, right=False)


# ============================================
# 6.2 PRECISIÓN DEL MODELO (vs Realidad)
# ============================================
print("\n" + "=" * 60)
print("PRECISIÓN DEL MODELO (vs Realidad)")
print("=" * 60)
...
for h in horizontes:
    ...
    # Check vs realidad (LO ÚNICO QUE IMPORTA)
    ...
    # Calcular métricas
    aciertos = (reporte_final[f'Check_{h}h'] == "✅").sum()
...
print("\n" + "=" * 60)
print("DISTRIBUCIÓN DE SEÑALES EN ANOMALÍAS")
print("=" * 60)
...
print("   1. Anomalía detectada (signal = LONG o SHORT)")
print("   2. KNN con mejora > 2% vs baseline")
print("   3. Confianza del modelo >= 60%")
print("   4. Alineamiento KNN-MR >= 60% (opcional)")

print("\n⚠️  GESTIÓN DE RIESGO:")
print("   • Stop loss: -2% del precio de entrada")
print("   • Take profit: Según horizonte óptimo")
print("   • Position size: Máximo 5% del capital por trade")
print("   • No operar más de 3 posiciones simultáneas")

print("\n🔍 PRÓXIMOS PASOS:")
print("   1. Implementar backtesting con P&L real")
print("   2. Calcular Sharpe ratio y max drawdown")
print("   3. Paper trading por 30 días")
print("   4. Ajustar parámetros según resultados")
...
# Para este contexto, asumimos que las celdas anteriores ya han ejecutado y estas variables están en el kernel.

# Re-calcular la última fila para asegurar que los datos sean los más recientes si el kernel se reinició.
# En un flujo continuo, estas variables ya deberían estar disponibles.
ultima_fila = df_features.tail(1)
es_anomalia_actual = ultima_fila['anomaly'].values[0]
fecha_actual = ultima_fila.index[0]
precio_actual = ultima_fila['Close'].values[0]
...
# Si hay una anomalía, añadir más detalles al mensaje
if es_anomalia_actual:
    signal_mr = ultima_fila['signal'].values[0]
    log_return = ultima_fila['log_return'].values[0]
    ...
    # Recalcular predicciones y confianzas para la última vela
    for h in horizontes:
        ...
        if h in metricas_detalladas:
            ...
            confiable_modelo = False # Si no hay métricas, asumimos no confiable
        ...
        tendencia = "ALCISTA" if pred == 1 else "BAJISTA"
        confianza = prob[pred] if pred < len(prob) else 0.0 # Asegurar índice válido
        ...
        # Normalizar signal_mr para comparación
        normalized_signal_mr = "ALCISTA" if signal_mr == "Long" else "BAJISTA"
        compatible = (tendencia == normalized_signal_mr) # Comparar predicción KNN con señal MR normalizada
        ...
        predicciones[h] = {
            ...
            "Nivel": nivel_confianza,
            "vs MeanRev": "✅ Alineado" if compatible else "⚠️ Divergente",
            "Modelo Confiable": "✅" if confiable_modelo else "❌"
        }

    # Calcular MR_Alignment_Score para la anomalía actual
    normalized_signal_mr_text = "ALCISTA" if signal_mr == "Long" else "BAJISTA"
    alignment_scores = []
    for h_key, p_val in predicciones.items():
        if p_val["Predicción"] == normalized_signal_mr_text:
    ...
    # Añadir detalles al mensaje_telegram_final
    mensaje_telegram_final += f"""

🎯 Señal Mean Reversion: {signal_mr}
🤝 Alineamiento KNN-MR: {current_mr_alignment_score:.0f}% ({current_mr_alignment_category})
"""

    mensaje_telegram_final += f"\n*PREDICCIONES POR HORIZONTE:*\n"
    predicciones_summary_lines = []
    for h, p_data in predicciones.items():
        # Simplificado según la petición del usuario
        predicciones_summary_lines.append(
            f"  *{h}h:* Pred: {p_data['Predicción']}"
        )
    mensaje_telegram_final += "\n".join(predicciones_summary_lines)


else:
    mensaje_telegram_final += "\n💡 No se requiere análisis detallado.\n   El mercado opera dentro de parámetros normales.\n"
